...
Branch.prototype.draw = function () {
  if (this.progress < this.length) {
    ...
    branches.push(
      new Branch(x2, y2, this.length * 0.72, this.angle - Math.PI / 6, this.depth - 1),
      new Branch(x2, y2, this.length * 0.72, this.angle + Math.PI / 6, this.depth - 1)
    );
  }
};
...
branches.push(
  new Branch(canvas.width / 2, canvas.height, 130, -Math.PI / 2, 10)
);
